...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProductWithNoWuantityAvailable = products.filter(
      product => existentProducts.filter(p => p.id == product.id)[0].quantity < product.quantity,
    );

    if(findProductWithNoWuantityAvailable.length) {
      throw new AppError(`The quantity ${findProductWithNoWuantityAvailable[0].quantity} is not available for ${findProductWithNoWuantityAvailable[0].id}`);
    }
